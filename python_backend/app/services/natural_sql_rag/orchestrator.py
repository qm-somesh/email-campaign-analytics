...
- If user wants 'low click rates', 'lowest click rates', 'least click rates', 'worst click rates': ADD 'HAVING SUM(CASE WHEN st.Status = ''delivered'' THEN 1 ELSE 0 END) > 0 AND (CAST(SUM(CASE WHEN st.Status = ''clicked'' THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(SUM(CASE WHEN st.Status = ''delivered'' THEN 1 ELSE 0 END), 0)) <= 0.05' AND ORDER BY (CAST(SUM(CASE WHEN st.Status = 'clicked' THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(SUM(CASE WHEN st.Status = 'delivered' THEN 1 ELSE 0 END), 0)) ASC
- If user wants 'low bounce rates', 'best delivery rates': ADD 'HAVING COUNT(DISTINCT eo.EmailOutboxId) >= 10 AND (CAST(SUM(CASE WHEN st.Status IN (''bounced'', ''failed'') THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(COUNT(DISTINCT eo.EmailOutboxId), 0)) <= 0.05' AND ORDER BY (CAST(SUM(CASE WHEN st.Status IN ('bounced', 'failed') THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(COUNT(DISTINCT eo.EmailOutboxId), 0)) ASC
- If user wants 'campaigns with low bounce rates' or 'good delivery rates': ADD 'HAVING COUNT(DISTINCT eo.EmailOutboxId) >= 10 AND (CAST(SUM(CASE WHEN st.Status IN (''bounced'', ''failed'') THEN 1 ELSE 0 END) AS FLOAT) / NULLIF(COUNT(DISTINCT eo.EmailOutboxId), 0)) <= 0.05'
